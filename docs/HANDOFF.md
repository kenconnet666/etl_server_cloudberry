...
- **table snapshot 中断恢复** ✅ exported snapshot 随源连接结束而失效，因此明确禁止把旧 page cursor 与新 source snapshot 拼接。新 lease 只允许重置仍处于 `snapshotting` 的 Reload/Add group，且 manifest 必须与 transition 的 relation/generation 一一对应；shadow、progress、manifest 删除与 transition 回到 `pending`/采用当前 fence 在同一 target 事务完成。`catching_up`/`cutover_pending` 不依赖已失效的 source snapshot，拒绝走此重置路径。runtime 尚未调用该恢复入口；新的 snapshot slot、WAL catch-up、reconciliation 和 executor 仍未完成。
