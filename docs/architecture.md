...
- 状态：目标架构，已接受
- 日期：2026-07-20
- 当前实现状态：仓库仍处于初始化阶段。本文描述交付目标和实现约束，不代表功能已经完成。
...
```text
crates/
  app/                 binary、配置装配、supervisor、HTTP/SSE
  cdc-core/            领域类型、LSN、generation、事件、状态机
  source-postgres/     catalog、snapshot、pgoutput、DDL、source HA
  source-citus/        topology、逐节点 reader、拓扑 generation
  metadata/            control/source/target metadata 与 migration
  target-cloudberry/   类型 DDL、COPY、staging、apply、rebuild
  reconciliation/      count、PK 分块 hash、repair plan
web/                   Svelte 5 + TypeScript + Vite
tests/
  integration/         PG18/Cloudberry/Citus 真实服务测试
  fault/               crash、WAL 丢失、网络分区、重复重放
  compatibility/       类型和 DDL 矩阵
docs/
  adr/                 已接受的架构决策
```

依赖方向：适配器 crate 依赖 `cdc-core`，`cdc-core` 不依赖数据库驱动、HTTP 或前端。跨 crate 共享的类型只有稳定领域概念；某数据库专有 SQL、OID 和错误不能泄漏进核心层。先选择性吸收 Supabase ETL 等项目中经验证的思路和协议测试，不复制其 workspace 结构，也不依赖滚动 `main`。
...
预检结果按表记录。默认表级 blocked 不阻塞其他合格表；会破坏全库 checkpoint 或 topology 覆盖的错误阻塞整个 pipeline。
...
Citus reader 连接 coordinator 和每个 active worker。每个物理节点有独立 publication/slot/reader/checkpoint：
...
Citus 14.1 CDC 是 upstream preview 功能，因此采取能力白名单：首批生产支持只有含 distribution key 主键的 hash-distributed row table。reference、coordinator-local 和 single-shard table 有设计路径，但必须分别通过真实集群的快照、增量、rebalance、failover 和重复事件矩阵后才可解锁。columnar、schema-sharded、append/range distributed 和复杂分布式 partition 默认拒绝。
